...
<div class="container" style="max-width: 900px;">
    ...
    <div class="card-box p-3">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <small class="text-secondary">백신 접종 내역</small>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex justify-content-between align-items-end border-top pt-3">
            <ul class="nav nav-pills">
                <li class="nav-item"><a class="nav-link" href="/heat/list?childId=${childId}&childName=${childName}">체온</a></li>
            ...
            <div class="profile-wrap d-flex gap-1">
                <a href="/vaccine/list?childId=0&childName=${sessionScope.userName}" class="profile-sm-btn ${childId == 0 ? 'active' : ''}">나</a>
                <c:forEach var="c" items="${childList}">
                    <a href="/vaccine/list?childId=${c.childId}&childName=${c.childName}" class="profile-sm-btn ${childId == c.childId ? 'active' : ''}">${c.childName}</a>
                </c:forEach>
    ...
    <div class="d-flex justify-content-end gap-2 my-3">
        ...
        <button class="btn btn-vaccine-main" data-bs-toggle="modal" data-bs-target="#addModal">
            <i class="fa-solid fa-plus"></i> 접종 일정 추가
        </button>
    </div>
    ...
    <c:if test="${totalCount > 0}">
        <div class="row mb-3">
            <div class="col-md-6">
                <div class="card-box white p-3 h-100 d-flex flex-column align-items-center justify-content-center">
                    <h6 class="fw-bold mb-3">접종 달성률</h6>
                    <div style="width: 200px; height: 200px; position: relative;">
                        <canvas id="vaccineChart"></canvas>
                        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center;">
                            <span class="fs-4 fw-bold text-primary" style="color:#1e293b !important;">
            ...
            <div class="col-md-6">
                <div class="card-box white p-3 h-100 d-flex flex-column justify-content-center">
    ...
    <div class="card-box p-3">
        <h6 class="fw-bold mb-3 d-flex align-items-center gap-2">
            <i class="fa-solid fa-syringe icon-blue"></i>
            접종 상세 내역
        </h6>
        <table class="table table-hover text-center align-middle">
            ...
            <c:choose>
                <c:when test="${empty list}">
                    <tr><td colspan="5" class="py-5 text-secondary">접종 내역이 없습니다.</td></tr>
                </c:when>
                <c:otherwise>
                    <c:forEach var="v" items="${list}">
                        <tr class="record-row"
                            ...
                            <td>
                                ...
                                <c:if test="${v.status ne 'Y' and not empty v.dayDiff}">
                                    <span class="badge-status badge-dday">D-${v.dayDiff}</span>
                                </c:if>
                            </td>
                            <td>
                                <c:choose>
                                ...
                                <c:choose>
                                    <c:when test="${v.status ne 'Y'}">
                                        <a href="/vaccine/complete?vaccineNo=${v.vaccineNo}&childId=${childId}&childName=${childName}"
                                           class="btn-action btn-vaccine">접종</a>
                                    </c:when>
                                    <c:otherwise>
                                        <a href="/vaccine/complete?vaccineNo=${v.vaccineNo}&childId=${childId}&childName=${childName}"
                                           class="btn-action btn-cancel">미접종</a>
                                    </c:otherwise>
                                </c:choose>
        ...
        <form action="/vaccine/add" method="post" class="w-100">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3"><label class="form-label">예정일</label><input type="text" name="inoculationDate" class="form-control datepicker" required></div>
                </div>
        ...
        <form action="/vaccine/update" method="post" class="w-100">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3"><label class="form-label">예정일</label><input type="text" name="inoculationDate" id="edit_date" class="form-control datepicker" required></div>
                </div>
